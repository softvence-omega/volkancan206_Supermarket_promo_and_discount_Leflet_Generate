...
from app.config import LOGO_DIR, PRODUCT_DIR, GENERATED_DIR
from app.schemas.Campaign_Info import CampaignRequest
from app.services.leaflet_generator import generate_leaflet  # <-- Gemini Generator Import
...
def campaign_generate(request: CampaignRequest):
    try:
        ...
        for product in products:
            ...
            product_dict = product.model_dump()
            product_dict["image_url"] = product_image_path   # local path update
            updated_products.append(product_dict)

        # --- Prepare final campaign request object ---
        request_dict = request.model_dump()
        request_dict["products"] = updated_products
        request_dict["supermarket_logo_url"] = logo_path   # update with local logo path

        # --- Call Gemini Leaflet Generator ---
        logger.info(f"Calling Gemini Leaflet Generator with {len(updated_products)} products...")
        pdf_path = generate_leaflet(request)   # direct call to leaflet_generator
        ...
        products=[
            {
                ...
                "new_price": 3.5,
                "discount": 30,
                "image_url": "./temp/product_images/apple.png",
                "currency": "$"
            },
            {
                "name": "Rice",
                ...
                "new_price": 15.0,
                "discount": 25,
                "image_url": "./temp/product_images/rice.png",
                "currency": "$"
            },
        ],
        products_per_page=1,
